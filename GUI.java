...
public class GUI {
	...
	public void openSignInForm(JFrame frame, Login_Account user) {
		...
		// main screen picture
		JLabel imgLabel = new JLabel(new ImageIcon( System.getProperty("user.dir") + "\\src\\def_pkg\\BankIcon.jpg"));
		imgLabel.setBounds(20,24,450, 400);
		f.add(imgLabel);
		
		//---------------------------------Creating Labels------------------------------------------		
		JLabel l_hSignIn = new JLabel("Sign In");
		l_hSignIn.setFont( l_hSignIn.getFont().deriveFont(28f) );
		...
		l_Signup.setBounds(480,380,150, 40);
		f.add(l_Signup);
		...
		btn_SignIn.setBackground( new Color(0, 104, 153) );
		...
		btn_SignUp.setBackground( new Color(0, 104, 153) );
		btn_SignUp.setForeground(Color.white);
		btn_SignUp.setBounds(620,380,110, 40);
		f.add(btn_SignUp);
				...
				if( user.getLoginId().compareTo("") == 0 ) {
					...
					if( user.getType().compareTo("Client") == 0 ) {
						...
						if ( Integer.valueOf(account.getStatus()) == 0 ) 
							...
							JOptionPane.showMessageDialog(f, "This account enter is blocked. You may not login at this time.");
						}
						else {
	...
	// sign up form 1, getting account# and SSID# from user
	void open_Signup_form_1(JFrame frame, Login_Account user)
	{
		...
		JLabel I_SSID = new JLabel("Enter the SSID:");
		I_SSID.setBounds(200, 180, 100, 90);
		f.add(I_SSID);
		...
		btn_Verify_Account.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
					...
					int verify_status = user.verify_account(tf_AccountNo.getText(), tf_SSID.getText());
					System.out.println("Verify Status Value: " + verify_status );
					if (verify_status == -1)
					{
						JOptionPane.showMessageDialog(f, "There is no account in our database that matches current credentials.");
						frame.remove(f);
						frame.repaint();
						frame.validate();
						openSignInForm(frame, user); 
					...
					{
						JOptionPane.showMessageDialog(f, "A bank account is has already been made for this account.");
					...
					{
						JOptionPane.showMessageDialog(f, "No account found with this SSID.");
						// **************************************
						System.out.println("The SSID did not match with any client.");
						frame.remove(f);
						frame.repaint();
						frame.validate();
						openSignInForm(frame, user);
	...
	void open_Signup_form_2(JFrame frame, Login_Account user, String acc_num)
	{
			...
			btn_create_login_Account.setBackground( new Color(0, 104, 153) );
	...
	void openClientMenu(JFrame frame, Client client, Bank_Account account) {
		...
		btnLogOut.setBackground( new Color(0, 104, 153) );
		...
		lDesg.setFont(lDesg.getFont().deriveFont(14f));					// Might need to adjust the font
		...
		btnAccInfo.setBackground( new Color(0, 104, 153) );
		...
		btnCCWithdrawal.setBackground( new Color(0, 104, 153) );
		...
		btnTransferMoney.setBackground( new Color(0, 104, 153) );
		...
		btnEStatement.setBackground( new Color(0, 104, 153) );
		...
		btnChangePass.setBackground( new Color(0, 104, 153) );
	...
	void open_info_page(JFrame frame, Client client, Bank_Account account)
	{
		...
		btn_sign_out.setBackground( new Color(0, 104, 153) );
	...
	void open_cardless_page_1(JFrame frame, Client client, Bank_Account account)
	{
		...
		btn_create_cardless_entry.setBackground( new Color(0, 104, 153) );
		...
		btn_mm.setBackground( new Color(0, 104, 153) );
	...
	void openTransferMoneyForm(JFrame frame, Client client, Bank_Account account) {
		...
		btn_Transfer.setBackground( new Color(0, 104, 153) );
	...
	void eStatement2(JFrame frame, Client client, Bank_Account account) {
		...
		btnDownload.setBackground( new Color(0, 104, 153) );
		...
		btnMM.setBackground( new Color(0, 104, 153) );
	...
	void open_change_password_form(JFrame frame, Client client, Bank_Account account)
	{
		...
		btn_change_password.setBackground( new Color(0, 104, 153) );
	...
	void openManagerMenu(JFrame frame, Manager manager) {
		...
		btnBlockAcc.setBackground( new Color(0, 104, 153) );
		...
		btnCloseAcc.setBackground( new Color(0, 104, 153) );
		...
		btnSearchAcc.setBackground( new Color(0, 104, 153) );
		...
		btnManageCard.setBackground( new Color(0, 104, 153) );
		...
		btnUpdateInfo.setBackground( new Color(0, 104, 153) );
	...
	public void openCreateAccountForm(JFrame frame, Manager manager) {
		...
		f.add(l_motherName);
		JLabel l_ssid = new JLabel("SSID: ");
		l_ssid.setBounds(100,200,100, 40);
		f.add(l_ssid);
		...
		f.add(tf_motherName);
		JTextField tf_ssid = new JTextField();
		tf_ssid.setBounds(200,200,180, 25);
		f.add(tf_ssid);
		...
		btn_create.setBackground( new Color(0, 104, 153) );
		...
		btn_create.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Client newClient = new Client( "",
						...
						tf_ssid.getText(), tf_dob.getText(), tf_phone.getText(), tf_email.getText(),
						tf_address.getText()
						);
				int res =  manager.createAccount(newClient, jcb_types.getSelectedItem().toString());
				if( res == 1 ) {
					String id_ = manager.getAccNum( tf_ssid.getText() );
					...
					JOptionPane.showMessageDialog(f, "Another account with this SSID number exists");
				else
	...
	void open_manage_card_page(JFrame frame, Manager manager)
	{
		...
		//		Account Opening Date
		JLabel l_cnic = new JLabel("Enter SSID of account holder");
		...
		btn_block_card.setBackground( new Color(0, 104, 153) );
		...
		btn_unblock_card.setBackground( new Color(0, 104, 153) );
	...
	void open_block_unblock_account_page(JFrame frame, Manager manager)
	{
		...
		JLabel l_cnic = new JLabel("Enter SSID:");
		...
		JTextField tf_ssid = new JTextField();
		tf_ssid.setBounds(400,200,200, 25);
		f.add(tf_ssid);
		
		//------------------------------------Creating buttons---------------------------------------------------				
		JButton btn_block_account = new JButton("Block Account");
		btn_block_account.setBackground( new Color(0, 104, 153) );
		...
		btn_unblock_account.setBackground( new Color(0, 104, 153) );
		...
		btn_block_account.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				//System.out.println("Request received of blocking account with acc num:"+acc_num+" and ssid:"+ssid);
				
				int block_status = manager.block_account( Integer.parseInt(tf_account_no.getText()), tf_ssid.getText() );
		...
		btn_unblock_account.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int unblock_status = manager.unblock_account( Integer.parseInt(tf_account_no.getText()), tf_ssid.getText() );
	...
	void open_close_account_page(JFrame frame, Manager manager)
	{
		...
		btn_close_account.setBackground( new Color(0, 104, 153) );
	...
	// In this screen manager will enter the client's account number and ssid number for search
	void searchAccount(JFrame frame, Manager manager) {
		...
		// Client CNIC
		JLabel lCSSID = new JLabel("SSID: ");
		lCSSID.setBounds(150,200,250, 40);
		lCSSID.setVisible(false);
		f.add(lCSSID);
		...
		btnSearch.setBackground( new Color(0, 104, 153) );
		...
		btnSearch.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if( (account.getAccountNum().compareTo("") == 0) || (client.getSSID().compareTo("") == 0 ) ) {
					...
					else if (client.getSSID().compareTo("") == 0 ) {
				...
				// Both client and his account was found
					...
					lCSSID.setText("SSID:  "+client.getSSID() );
	...
	void updateClientInfo(JFrame frame, Manager manager) {
		...
		btnUpdate.setBackground( new Color(0, 104, 153) );
	...
	void openAccountantMenu(JFrame frame, Accountant accountant) {
		...
		lDesg.setFont( lDesg.getFont().deriveFont(28f) );
		lDesg.setBounds(305,80,150, 40);
		f.add(lDesg);
		
		// Deposit Cash button
		JButton btnDeposit = new JButton("Deposit Cash");
		btnDeposit.setBackground( new Color(0, 104, 153) );
		...
		btnWithdraw.setBackground( new Color(0, 104, 153) );
		btnWithdraw.setForeground(Color.white);
		btnWithdraw.setBounds(300,180,150, 40);
		f.add(btnWithdraw);
		
		// Check Deposit button
		JButton btnChequeDeposit = new JButton("Check Deposit");
		btnChequeDeposit.setBackground( new Color(0, 104, 153) );
		...
		// function to be executed when Check Deposit Button is clicked
	...
	void searchForm(JFrame frame, Accountant accountant, int case_) {
		...
		// SSID Number Label
		JLabel lSSID = new JLabel("Enter SSID number:");
		lSSID.setBounds(150,180,150, 40);
		f.add(lSSID);
		
		// SSID Number Text Field
		JTextField tfSSID = new JTextField();
		tfSSID.setBounds(150,220,180, 25);
		f.add(tfSSID);
		...
		btnSearch.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				Bank_Account account = accountant.searchAccount1( tfAccNum.getText(), tfSSID.getText() );
				Client client =  accountant.searchClient1( tfAccNum.getText(), tfSSID.getText()  );
	...
	void depositCash(JFrame frame, Accountant accountant, Client client, Bank_Account account) {
		...
		// SSID Label
		JLabel lSSID = new JLabel( "SSID: " + client.getSSID() );
		lSSID.setBounds(150,130,150, 40);
		f.add(lSSID);
	...
	void eStatement(JFrame frame, Accountant accountant, Client client, Bank_Account account) {
		...
		// Function to be executed when Get EStatement Button is clicked
		btnEStatement.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				tableModel.setRowCount(0);
	...
	void chequeDeposit(JFrame frame, Accountant accountant) {
		...
		// Check Deposit Label
		JLabel lCheque = new JLabel("Check Deposit");
		...
		// Cheque Number Label
		JLabel lChequeNum = new JLabel("Enter Check number:");
		...
		btnDeposit.addActionListener( new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if( Integer.valueOf( tfAmount.getText() ) > 0 ) {
					...
					{
						if( z == 1 ) 
							JOptionPane.showMessageDialog(f, "Check Deposit Successful");
						else 
							JOptionPane.showMessageDialog(f, "Check Deposit Failed");
						frame.remove(f);
						frame.repaint();
						frame.validate();
						openAccountantMenu(frame, accountant);	
